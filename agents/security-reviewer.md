...
Subsections below are in alphabetical order by pack name (Django/Python, Express/Node.js, iOS/Swift, Phoenix/Elixir, Rails/Ruby, React/Next.js) to avoid implying any single stack is the canonical example. The framework-agnostic Web defense-in-depth pack follows the framework packs as a structural sibling — it rides on top of whichever framework is active. Adding a seventh framework pack (Spring, Gin, Laravel, FastAPI, etc.) follows the same template.
