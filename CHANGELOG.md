...
- **React / Next.js framework rule pack.** New pack at `agents/security-reviewer.md` between the Rails pack and the Web defense-in-depth pack. Activation: `.jsx` / `.tsx` / `.js` / `.ts` extensions AND import of `react` / `react-dom` / any `next/*` subpath (`next/router`, `next/image`, `next/server`, `next/link`, `next/navigation`, `next/headers`), OR file under `pages/api/**` or `app/**/route.{js,ts}`. Five rules: (1) `dangerouslySetInnerHTML` with user HTML (xss_or_code_exec, CWE-79) — non-trigger for DOMPurify / sanitize-html / literal constants; (2) Next.js API route without auth check (authentication, CWE-306) — three-AND condition (state-changing op + no getServerSession/auth helper + no middleware.ts gate), non-trigger for GET-only public-data handlers; (3) `redirect()` / `rewrite()` with user-controlled destination (input_validation, CWE-601) — non-trigger for hardcoded paths and next-auth `callbackUrl`; (4) `<a href={user_url}>` / `<img src={user_url}>` with `javascript:` or `data:` scheme bypass (xss_or_code_exec, CWE-79) — non-trigger for relative URLs and scheme-allow-list-validated; (5) `getServerSideProps` / `getStaticProps` / Route Handler leaking secrets via props (data_exposure, CWE-200) — non-trigger for `NEXT_PUBLIC_*` env vars and sanitized DTOs. Comes with `react_dangerously_set_inner_html.jsx`, `nextjs_api_no_auth.ts`, `nextjs_redirect_user_input.ts`, `react_href_javascript_uri.jsx`, `nextjs_getserversideprops_leak.tsx`. Prior art: eslint-plugin-react, Next.js Lint — pattern-based; the semantic-analysis differentiator is prop-flow tracking from API to component to DOM plus auth-gate detection across `pages/api/` route handlers.
...
- **Rails pack expansion: `connection.execute` interpolation, missing `authenticate_user!`, unfiltered JSON model render.** Three new rules: (1) `ActiveRecord::Base.connection.execute("... #{params[:y]}")` / `.exec_query` with interpolation (CWE-89, A03:2021) mapped to `injection` — a different sink from `find_by_sql` covering the lower-level connection adapter. (2) Missing `before_action :authenticate_user!` on a controller that defines state-changing actions (CWE-306, A07:2021) mapped to `authentication`, with explicit non-trigger guidance for public-auth-flow controllers (`SessionsController#create`, `RegistrationsController#create`, `PasswordResetsController`). (3) `render json: @user` (or `render json: @users` / `respond_to` JSON path) where the model has `password_digest`, `reset_password_token`, `api_token`, `two_factor_secret`, `encrypted_*`, or any `*_token` / `*_secret` field, without `only:` / `except:` / serializer (CWE-200, A04:2021) mapped to `data_exposure`. Comes with `rails_connection_execute.rb`, `rails_missing_auth.rb`, `rails_json_leak.rb`. The Rails open-redirect and Marshal.load rules were added in W465 and W466 respectively, not duplicated here.
- **Phoenix pack expansion: `Phoenix.Token` no `max_age`, `System.cmd` shell wrapper, LiveView upload guards.** Three new rules: (1) `Phoenix.Token.verify` without `max_age` or with `max_age: :infinity` (CWE-613, A07:2021) mapped to `authentication`, the canonical token-replay vector. (2) `System.cmd("sh", ["-c", ...])` or `:os.cmd` with user input concatenated into the shell command (CWE-78, A03:2021) mapped to `injection`. (3) LiveView `allow_upload(:any)` without `max_file_size:` / content-type re-check (CWE-434, A04:2021) mapped to `input_validation`. Each has explicit non-trigger guidance (the `to:` redirect form, `System.cmd("convert", [argv])`, `allow_upload` with both `accept:` and `max_file_size:` set). Comes with `phoenix_token_no_max_age.ex`, `phoenix_system_cmd_shell.ex`, and `phoenix_liveview_upload.ex` fixtures. The Phoenix open-redirect rule was added in the parallel open-redirect task and is not duplicated here.
- **Django pack expansion: SSRF, DRF `ModelSerializer fields='__all__'`, ALLOWED_HOSTS / SECURE_* hardening.** Three changes to the Django pack: (1) new SSRF rule (CWE-918, A10:2021) mapped to `input_validation` covering `requests.get(url)`, `urllib.request.urlopen(url)`, and `httpx.AsyncClient().get(url)` where `url` traces to `request.*` — realistic worst case is cloud-metadata exfiltration via `169.254.169.254`. (2) new DRF mass-assignment rule (CWE-915, A04:2021) mapped to `authorization` covering `serializers.ModelSerializer` with `fields = '__all__'` on a model that has privileged fields (`is_staff`, `is_superuser`, `is_admin`, `role`, `owner_id`). (3) extended the existing `DEBUG = True` rule into a combined "production-bound settings hardening" rule that also flags `ALLOWED_HOSTS = ['*']` and missing `SECURE_SSL_REDIRECT` / `SESSION_COOKIE_SECURE` / `CSRF_COOKIE_SECURE` / `SECURE_HSTS_SECONDS`. All three rules have explicit non-trigger guidance (validated SSRF allow-lists, explicit DRF `fields = [...]`, dev/test settings files). Comes with `django_ssrf.py`, `django_drf_mass_assignment.py`, and `django_insecure_settings.py` fixtures.
- **Deserialization rules for Django and Rails packs** (CWE-502, A08:2021). Both map to `xss_or_code_exec`. Django flags `pickle.loads(...)` on attacker-influenced bytes, `yaml.load(...)` without `Loader=yaml.SafeLoader`, `shelve.open(...)` on attacker-controlled paths, and `django.core.signing.loads(token)` without an explicit `max_age` on a value sourced from `request.*`. Rails flags `Marshal.load(params[...])`, `YAML.load(...)` without `permitted_classes:`, and `YAML.unsafe_load(...)`. Negative cases explicit in both rule rows: `yaml.safe_load`/`json.loads`/`YAML.safe_load`/`JSON.parse` and signing.loads with both `max_age` and `salt`. Comes with `test/fixtures/django_deserialization.py` and `test/fixtures/rails_deserialization.rb`. Closes a high-recall CVE class for both ecosystems (Rails CVE-2013-0156, CVE-2022-32224; Python's canonical pickle.loads RCE).
- **Open-redirect rules across Django, Phoenix, and Rails packs** (CWE-601, A01:2021). Each pack gains one rule mapped to `input_validation`: Django flags `HttpResponseRedirect(request.GET['next'])` / `redirect(request.POST['url'])` without `django.utils.http.url_has_allowed_host_and_scheme`; Phoenix flags `redirect(conn, external: params['url'])` (the `external:` key bypasses same-origin); Rails flags `redirect_to params[:url], allow_other_host: true` and similar overrides of the Rails 7+ default. Each rule has explicit non-trigger guidance (validated redirects, internal route helpers, `to:` key for Phoenix). Comes with three positive-control fixtures: `django_open_redirect.py`, `phoenix_open_redirect.ex`, `rails_open_redirect.rb`. Closes the most-flagged class in Brakeman's Rails output and equivalent gaps in the Django and Phoenix packs.
- **Negative-control fixtures.** Three look-alike-but-safe fixtures added to `test/fixtures/`: `phoenix_changeset_safe.ex` (`cast/3` with explicit `[:email, :name]` allow-list excluding `:role` and `:is_admin` — must NOT trigger mass-assignment), `phoenix_fragment_safe.ex` (`fragment("? = ?", field, ^user_input)` positional-binding form with the `^` pin — must NOT trigger fragment-injection), `rails_html_safe_constant.rb` (`.html_safe` on string literals and an allow-list-keyed lookup table — must NOT trigger XSS). The eval runner gained a `→ NONE` syntax in `EXPECTED.md` for zero-findings assertions; `compare_finding` was extended to fail when any finding fires on a negative-control fixture, so over-firing regressions are detected mechanically (today the eval could only catch missing findings, not over-firing).
